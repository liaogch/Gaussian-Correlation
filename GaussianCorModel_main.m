...
sigma_2 = 0:0.01:7;
sigma_1 = 0:0.01:7;
U_sum = zeros(length(sigma_1),length(sigma_2)); 
U_sum_max = 0;
sigma_opt = [0,0];

for i = 1:length(sigma_1)
    for j = 1:length(sigma_2)
        temp = Cal_Uti( 1,N, sigma_1(i),sigma_2(j),sigma_g,cor_var,r,R_max,r_d)+Cal_Uti( 2,N, sigma_2(i),sigma_1(j),sigma_g,cor_var,r,R_max,r_d);
        if temp >= U_sum_max
            U_sum_max = temp;
            sigma_opt = [sigma_1(i),sigma_2(j)];
        end
        U_sum(i,j)=temp;
    end
end
