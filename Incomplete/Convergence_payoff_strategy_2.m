...
P = cell(N,length(sigma),state_size);
...
for i = 1:iteration
    for j = 1:N
        sigma_action(j,i) = Choose_action (sigma,P{j}(:,last_state_index(j,i)));
    end
    for j = 1:N
        ...
        P_temp = P{j};
        P_temp(:,last_state_index(j,i)) = P{j}(:,last_state_index(j,i)) + b*normalized_uti(j,i)*(e-P{j}(:,last_state_index(j,i)));
        P{j} = P_temp;
    end
